...
```{r, message = FALSE}
# dat <- read_csv(here("Data", "Data_raw_crosschecked.csv")) #raw data was used for data processing below
...
### Trait and variation cause

```{r}
freq2 <- as.data.frame(table(dat$PC_type, dat$PC_variation_cause)) %>%
  rename("Trait" = Var1, "Variation_cause" = Var2)
...
levels(freq2$Variation_cause) <- list("natural" = "1", "environmental plasticity" = "2", "age" = "3", "direct manipulation" = "4", "selection" = "5", "other/unclear" = "6")

p2 <- ggplot(data = freq2, aes(axis1 = Variation_cause, axis2 = Trait, y = Freq)) +
    geom_alluvium(aes(fill = Variation_cause)) + geom_flow() + geom_stratum(aes(fill = Variation_cause)) +
...
theme_void() + theme(legend.position = "none", plot.title = element_text(hjust = 0,
    ...
    plot.margin = unit(c(1, 1, 0, 1), "cm")) + scale_x_discrete(limits = c("Variation cause", "Postcopulatory trait"), expand = c(0.15, 0.05), position = "top") 


```


### Fertilisation mode, Fertilisation method, mating order

```{r}
freq3 <- as.data.frame(table(dat$Fertilisation_mode, dat$Fertilisation_methods, dat$Competition_order)) %>%
  rename("Fertilisation_mode" = Var1, "Fertilisation_method" = Var2, "Mating_order" = Var3)

levels(freq3$Fertilisation_mode) <- list("internal" = "1", "external" = "2")

levels(freq3$Fertilisation_method) <- list("natural" = "1", "artificial fertilisation" = "2", "other/unclear" = "3")

levels(freq3$Mating_order) <- list("P1" = "1", "P2" = "2", "random mixing" = "3", "P1 and P2 (mating order controlled for)" = "4", "other/unclear" = "5")

p3 <- ggplot(data = freq3, aes(axis1 = Fertilisation_mode, axis2 = Fertilisation_method, axis3 = Mating_order, y = Freq)) +
...
theme_void() + theme(legend.position = "none", plot.title = element_text(hjust = 0,
    vjust = 3), axis.title.x = element_text(), axis.text.x = element_text(face = "bold"),
    plot.margin = unit(c(1, 1, 0, 1), "cm")) + scale_x_discrete(limits = c("Fertilisation mode", "Fertilisation method", "Mating order"), expand = c(0.15, 0.05), position = "top") 

```

```{r, warning= FALSE}

Fig2 <- (p2 + p3) + plot_annotation(tag_levels = "A")

Fig2

#TODO need to clean up this figure
...
#TODO why don't this figure work?? 
```
