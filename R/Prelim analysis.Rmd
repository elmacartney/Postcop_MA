...
# dat <- read_csv(here("Data", "Data_raw_crosschecked_variationrecoded_totalspermlength.csv")) #raw data was used for data processing below
...
# 
# levels(freq1$Variation_cause) <- list("genetic + environment" = "1", "environmental plasticity" = "2", "age" = "3", "direct manipulation" = "4", "genetic differences" = "5", "unclear/other" = "6")
# 
...
#TODO why does this keep adding a bunch of noise on alluvial plot??

freq1 <- as.data.frame(table(dat$Fertilisation_mode, dat$Competition_order, dat$PC_type, dat$PC_variation_cause2)) %>%
...
#sigma2 effect size ID
# 0.0959 / (0.0959 + 0.0546)

#sigma2 studyID
# 0.0546 / (0.0959 + 0.0546)


res1 <- mod_results(m1, mod = "PC_type", data = dat1, group = "Study_ID")

fig.4 <- orchard_plot(res1 , mod = "PC_type", group = "Study_ID", data = dat1, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh") +
...
m4a_MR <- rma.mv(yi = yi, V = VCV3, mod = ~PC_type-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res3 <- mod_results(m4a_MR, mod = "PC_type", data = dat3, group = "Study_ID")

fig.4c <- orchard_plot(res3, mod = "PC_type", group = "Study_ID", data = dat3, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45)) +
...
m4b_MR <- rma.mv(yi = yi, V = VCV4, mod = ~PC_type-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res4 <- mod_results(m4b_MR, mod = "PC_type", data = dat4, group = "Study_ID")

fig.4d <- orchard_plot(res4, mod = "PC_type", group = "Study_ID", data = dat4, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45)) +
...
round(i2_ml(m5), 2) #no phylogenetic signal

#including fert mode as a moderator

m5_MR <- rma.mv(yi = yi, V = VCV5, mod = ~Fertilisation_mode-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res5 <- mod_results(m5_MR, mod = "Fertilisation_mode", data = dat5, group = "Study_ID")

fig.5 <- orchard_plot(res5, mod = "Fertilisation_mode", group = "Study_ID", data = dat5, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45))

#including mating order

dat5b <- subset(dat5, Competition_order <4)

dat5b$Competition_order <- as.factor(dat5b$Competition_order)


VCV5b <- impute_covariance_matrix(vi = dat5b$vi, cluster = dat5b$Study_ID, r = 0.5)


m5_MRb <- rma.mv(yi = yi, V = VCV5b, mod = ~Competition_order-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res5b <- mod_results(m5_MRb, mod = "Competition_order", data = dat5b, group = "Study_ID")

levels(dat5b$Competition_order) <- list("P1" = "1", "P2" = "2", "random mixing" = "3")

fig.5b <- orchard_plot(m5_MRb, mod = "Competition_order", group = "Study_ID", data = dat5b, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45))

```

#### Sperm length (all measures)

```{r,}

dat6 <- subset(dat, PC_type == 3)

VCV6 <- impute_covariance_matrix(vi = dat6$vi, cluster = dat6$Study_ID, r = 0.5)

m6 <- rma.mv(yi = yi, V = VCV6, 
             random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
round(i2_ml(m6), 2) #Species non-phylogenetic signal explains a lot!... but it's not due to fertilisation mode
# 
# res0 <- mod_results(m6, mod = "1", data = dat6, group = "Study_ID")
# 
# fig.6a <- orchard_plot(res0, mod = "1", group = "Study_ID", data = dat6, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
#   theme(axis.text.y = element_text(angle = 45))

#including fert mode as a moderator

m6_MR <- rma.mv(yi = yi, V = VCV6, mod = ~Fertilisation_mode-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res6 <- mod_results(m6_MR, mod = "Fertilisation_mode", data = dat5, group = "Study_ID")

fig.6 <- orchard_plot(res6, mod = "Fertilisation_mode", group = "Study_ID", data = dat6, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45))

### including mating order as a moderator

dat6b <- subset(dat6, Competition_order <4)

dat6b$Competition_order <- as.factor(dat6b$Competition_order)

VCV6b <- impute_covariance_matrix(vi = dat6b$vi, cluster = dat6b$Study_ID, r = 0.5)

m6_MRb <- rma.mv(yi = yi, V = VCV6b, mod = ~Competition_order-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res6b <- mod_results(m6_MRb, mod = "Competition_order", data = dat6b, group = "Study_ID")

levels(dat6b$Competition_order) <- list("P1" = "1", "P2" = "2", "random mixing" = "3")

fig.6b <- orchard_plot(m6_MRb, mod = "Competition_order", group = "Study_ID", data = dat6b, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45))

```

#### Sperm length (just total)

```{r}
dat6 <- subset(dat, PC_type == 3)

dat6c <- subset(dat6, !(PC_type_notes %in% c("tail", "head", "midpiece")))

VCV6c <- impute_covariance_matrix(vi = dat6c$vi, cluster = dat6c$Study_ID, r = 0.5)
...
round(i2_ml(m6c), 2) #Species non-phylogenetic signal explains a lot!... but it's not due to fertilisation mode
...
res6c <- mod_results(m6_MRc, mod = "Fertilisation_mode", data = dat5, group = "Study_ID")

fig.6c <- orchard_plot(res6c, mod = "Fertilisation_mode", group = "Study_ID", data = dat6c, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45))

### including mating order as a moderator
## no P1
dat6d <- subset(dat6c, Competition_order <4)

dat6d$Competition_order <- as.factor(dat6d$Competition_order)

VCV6d <- impute_covariance_matrix(vi = dat6d$vi, cluster = dat6d$Study_ID, r = 0.5)

m6_MRd <- rma.mv(yi = yi, V = VCV6d, mod = ~Competition_order-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res6d <- mod_results(m6_MRd, mod = "Competition_order", data = dat6d, group = "Study_ID")

levels(dat6d$Competition_order) <- list("P1" = "1", "P2" = "2", "random mixing" = "3")

fig.6d <- orchard_plot(m6_MRd, mod = "Competition_order", group = "Study_ID", data = dat6d, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45))
...
summary(m7, transfm = "tanh")

round(i2_ml(m7), 2) #Species phylogeny explains a lot of variation in velocity

m7_MR <- rma.mv(yi = yi, V = VCV7, mod = ~Fertilisation_mode-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res7 <- mod_results(m7_MR, mod = "Fertilisation_mode", data = dat7, group = "Study_ID")

fig.7 <- orchard_plot(res7, mod = "Fertilisation_mode", group = "Study_ID", data = dat7, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45))

### including mating order as a moderator
## no P1
dat7b <- subset(dat7, Competition_order <4)

dat7b$Competition_order <- as.factor(dat7b$Competition_order)

VCV7b <- impute_covariance_matrix(vi = dat7b$vi, cluster = dat7b$Study_ID, r = 0.5)

m7_MRb <- rma.mv(yi = yi, V = VCV7, mod = ~Competition_order-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res7b <- mod_results(m7_MRb, mod = "Competition_order", data = dat7b, group = "Study_ID")

levels(dat7b$Competition_order) <- list("P1" = "1", "P2" = "2", "random mixing" = "3")

fig.7b <- orchard_plot(m7_MRb, mod = "Competition_order", group = "Study_ID", data = dat6d, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45))
...
summary(m8, transfm = "tanh")

round(i2_ml(m8), 2) #Species non-phylogeny explain a lot of variation

m8_MR <- rma.mv(yi = yi, V = VCV8, mod = ~Fertilisation_mode-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
summary(m8_MR)
r2_ml(m8_MR) ## fert mode explains 26% of total variation!!

res8 <- mod_results(m8_MR, mod = "Fertilisation_mode", data = dat8, group = "Study_ID")

fig.8 <- orchard_plot(res8, mod = "Fertilisation_mode", group = "Study_ID", data = dat8, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45))

### including mating order as a moderator
## no P1
dat8b <- subset(dat8, Competition_order <4)

dat8b$Competition_order <- as.factor(dat8b$Competition_order)

VCV8b <- impute_covariance_matrix(vi = dat8b$vi, cluster = dat8b$Study_ID, r = 0.5)

m8_MRb <- rma.mv(yi = yi, V = VCV8b, mod = ~Competition_order-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
#just including effect size ID rather than study as a random effect due to nearly non-identifiability between ES_ID and study

m8c <- rma.mv(yi = yi, V = VCV8, 
...
summary(m8c, transfm = "tanh")

round(i2_ml(m8c), 2) #Species non-phylogeny explain a lot of variation
...
dat5 <- subset(dat, PC_variation_cause2 !=6)

dat5$PC_variation_cause2<-as.factor(dat5$PC_variation_cause2)

levels(dat5$PC_variation_cause2) <- list("genetic + environmental" = "1", "environmental plasticity" = "2", "age" = "3", "direct manipulation" = "4", "genetic variation" = "5")

VCV5 <- impute_covariance_matrix(vi = dat5$vi, cluster = dat5$Study_ID, r = 0.5)

#TODO for some reason the variance VCV was preventing convergence
m2 <- rma.mv(yi = yi, V = vi, mod = ~PC_variation_cause2-1,  
             random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res5 <- mod_results(m2, mod = "PC_variation_cause2", data = dat5, group = "Study_ID")
...
### Only using studies of total sperm length
...
summary(m0a, transfm = "tanh") #TODO is the working?? values are the same. 

round(i2_ml(m0a), 2) #both species and phylo = 0% variation
...
res1 <- mod_results(m1a, mod = "PC_type", data = dat_SLtotala, group = "Study_ID")

figS4a <- orchard_plot(res1 , mod = "PC_type", group = "Study_ID", data = dat_SLtotala, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh") +
  theme(axis.text.y = element_text(angle = 45)) +
  ggtitle('Post-copulatory trait as a moderator in global model') +
    geom_signif(comparisons = list(c("Seminal fluid quantity", "Total ejaculate size", "Sperm normality", "Sperm velocity", "Sperm length", "Sperm number")),
              annotations = "*")
...
m4a <- rma.mv(yi = yi, V = VCV, mod = ~Fertilisation_mode-1,  
             random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res2 <- mod_results(m4a, mod = "Fertilisation_mode", data = dat_SLtotal, group = "Study_ID")

figS4b <- orchard_plot(res2, mod = "Fertilisation_mode", group = "Study_ID", data = dat_SLtotal, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+
  scale_colour_manual(values = c("grey34","grey34")) +
  scale_fill_manual(values=c("grey34","grey34"))  +
  theme(axis.text.y = element_text(angle = 45)) +
  ggtitle('Fertilisation mode as a moderator in global model') +
...
dat3a <- subset(dat_SLtotala, Fertilisation_mode == "internal")

VCV3a <- impute_covariance_matrix(vi = dat3a$vi, cluster = dat3a$Study_ID, r = 0.5)

dat3a$PC_type<-as.factor(dat3a$PC_type)
# 
levels(dat3a$PC_type) <- list("sperm number" = "1", "sperm length" = "3", "sperm velocity" = "4", "sperm normality" = "5", "total ejaculate size" = "6", "seminal fluid quantity" = "7", "accessory gland size" = "8")

m4a <- rma.mv(yi = yi, V = VCV3a, mod = ~PC_type-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res3 <- mod_results(m4a, mod = "PC_type", data = dat3a, group = "Study_ID")

figS4c <- orchard_plot(res3, mod = "PC_type", group = "Study_ID", data = dat3a, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45)) +
  ggtitle('Post-copulatory traits within internal fertilisers') +
    geom_signif(comparisons = list(c("Seminal fluid quantity", "Total ejaculate size", "Sperm normality", "Sperm velocity", "Sperm length", "Sperm number")),
              annotations = "*")
...
dat3b <- subset(dat_SLtotala, Fertilisation_mode == "external")

VCV3b <- impute_covariance_matrix(vi = dat3b$vi, cluster = dat3b$Study_ID, r = 0.5)

dat3b$PC_type<-as.factor(dat3b$PC_type)
# 
levels(dat3b$PC_type) <- list("sperm number" = "1", "sperm length" = "3", "sperm velocity" = "4", "sperm normality" = "5", "total ejaculate size" = "6", "seminal fluid quantity" = "7", "accessory gland size" = "8")


m4b <- rma.mv(yi = yi, V = VCV3b, mod = ~PC_type-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
res4 <- mod_results(m4b, mod = "PC_type", data = dat3b, group = "Study_ID")

figS4d <- orchard_plot(res4, mod = "PC_type", group = "Study_ID", data = dat3b, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45)) +
  ggtitle('Post-copulatory traits within external fertilisers') +
    geom_signif(comparisons = list(c("Seminal fluid quantity", "Total ejaculate size", "Sperm normality", "Sperm velocity", "Sperm length", "Sperm number")),
              annotations = "*")
...
### Fig.S4
```{r,}
figS4 <- ((figS4a + figS4b)/(figS4c + figS4d)) + plot_annotation(tag_levels = 'A')
figS4
```
