...
```{r,}
freq1 <- as.data.frame(table(dat$Fertilisation_mode, dat$PC_type, dat$PC_variation_cause)) %>%
...
#TODO why does the code I previously used for orchard plots not work??
...
fig.4a <- orchard_plot(m1, mod = "PC_type", group = "Study_ID", data = dat1, xlab = "zr", alpha=0.4) +
...
fig.4b <- orchard_plot(m4, mod = "Fertilisation_mode", group = "Study_ID", data = dat, xlab = "zr", alpha=0.4)+
...
fig.4c <- orchard_plot(m4a, mod = "PC_type", group = "Study_ID", data = dat3, xlab = "zr", alpha=0.4)+ ylim(-1,2) +
  theme(axis.text.y = element_text(angle = 45)) +
    geom_signif(comparisons = list(c("Seminal fluid quantity", "Total ejaculate size", "Sperm normality", "Sperm velocity", "Sperm length", "Sperm number")),
              annotations = "*")
...
fig.4d <- orchard_plot(m4b, mod = "PC_type", group = "Study_ID", data = dat4, xlab = "zr", alpha=0.4)+ ylim(-1,2) +
  theme(axis.text.y = element_text(angle = 45)) +
    geom_signif(comparisons = list(c("Seminal fluid quantity", "Total ejaculate size", "Sperm normality", "Sperm velocity", "Sperm length", "Sperm number")),
              annotations = "*")
...
levels(dat$PC_variation_cause) <- list("natural" = "1", "environmental plasticity" = "2", "age" = "3", "direct manipulation" = "4", "experimental selection" = "5", "unclear/other" = "6")
...
```{r,}
fig.5 <- orchard_plot(m2, mod = "PC_variation_cause", group = "Study_ID", data = dat, xlab = "zr", alpha=0.4) +
  theme(axis.text.y = element_text(angle = 45)) +
