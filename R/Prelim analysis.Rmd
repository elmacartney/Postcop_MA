...
for (i in 1:length(levels(dat$Study_ID))) {
    d <- dat %>%
        filter(Study_ID != levels(d$Study_ID)[i])

    VCV_Sb <- impute_covariance_matrix(vi = d$yi, cluster = d$Study_ID, r = 0.5)

    LeaveOneOut_effectsize[[i]] <- rma.mv(yi = yi, V = VCV_Sb, random = list(~1 |
        Study_ID, ~1 | Effectsize_ID, ~1 | Species), method = "REML", data = data[data$Study_ID !=
        levels(data$Study_ID)[i], ])
}
