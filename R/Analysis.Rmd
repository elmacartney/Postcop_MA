...
source(here("R/contrast_analysis.R"))
...
summary(m2b) # not signficantly different
...
```{r,}
m4_MRb <- rma.mv(yi = yi, V = VCV3, mod = ~Fertilisation_mode,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
```{r,}
m5_MRb <- rma.mv(yi = yi, V = VCV4, mod = ~Fertilisation_mode,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
m6_MRb <- rma.mv(yi = yi, V = VCV5, mod = ~Fertilisation_mode,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
funnel(mod_fm, xlab = "Residuals (Zr(", ylab = "Inverse standard error", yaxis = "seinv")
...
#contrast
m_fish_contrasta <- rma.mv(yi = yi, V = VCV_fisha, mod = ~PC_type,  
             random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
#contrast
m_fish_contrastb <- rma.mv(yi = yi, V = VCV, mod = ~Fertilisation_mode,  
             random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
m_fish_number1 <- rma.mv(yi = yi, V = VCV,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
# m_fish_contrastc <- rma.mv(yi = yi, V = vi, mod = ~Fertilisation_mode,  
#              random = list(~1|Study_ID,
#                           ~1|Effectsize_ID,
#                           ~1|Species,
#                          ~1|Phylogeny),
...
m_fish_length1 <- rma.mv(yi = yi, V = VCV,
             random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
#contrast
m_fish_contrastd <- rma.mv(yi = yi, V = VCV, mod = ~Fertilisation_mode,
             random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
summary(m_fish_contrastd)
```

##### Sperm velocity sub-analysis with fertilisation mode as moderator

```{r,}
dat_class_velocity <- subset(dat_Class, PC_type == 4)

VCV<- impute_covariance_matrix(vi = dat_class_velocity$vi, cluster = dat_class_velocity$Study_ID, r = 0.5)

m_fish_velocity1 <- rma.mv(yi = yi, V = VCV,
             random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
             R=list(Phylogeny = VCV_species),
             test = "t",
             data = dat_class_velocity)

round(i2_ml(m_fish_velocity1), 2)

#including fert mode as a moderator
m_fish_velocity <- rma.mv(yi = yi, V = VCV, mod = ~Fertilisation_mode-1,  
              random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
summary(m_fish_velocity)
r2_ml(m_fish_velocity)

#contrast
m_fish_contraste <- rma.mv(yi = yi, V = VCV, mod = ~Fertilisation_mode,
             random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species,
                         ~1|Phylogeny),
...
figS6a <- orchard_plot(res_fisha, mod = "PC_type", group = "Study_ID", data = dat_Classa, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh") +
  theme(axis.text.y = element_text(angle = 45)) +
...
figS6b <- orchard_plot(res_fishb, mod = "Fertilisation_mode", group = "Study_ID", data = dat_Classa, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh") +
  theme(axis.text.y = element_text(angle = 45)) +
  ggtitle('Fertilisation mode as a moderator in fish sub-analysis') +
    geom_signif(comparisons = list(c("internal", "external")),
              annotations = "*")

#sperm number sub-analysis
res_fish_number <- mod_results(m_fish_number, mod = "Fertilisation_mode", data = dat_class_number, group = "Study_ID")

figS6c <- orchard_plot(res_fish_number, mod = "Fertilisation_mode", group = "Study_ID", data = dat3, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45)) +
  ggtitle('Sperm number') +
    geom_signif(comparisons = list(c("internal", "external")),
              annotations = "*")
...
#sperm length sub-analysis
res_fish_length <- mod_results(m_fish_length, mod = "Fertilisation_mode", data = dat_class_length, group = "Study_ID")

figS6d <- orchard_plot(res_fish_length, mod = "Fertilisation_mode", group = "Study_ID", data = dat3, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45)) +
  ggtitle('Sperm length') +
    geom_signif(comparisons = list(c("internal", "external")),
              annotations = "*")

#sperm velocity sub-analysis
res_fish_velocity <- mod_results(m_fish_velocity, mod = "Fertilisation_mode", data = dat_class_velocity, group = "Study_ID")

figS6e <- orchard_plot(res_fish_velocity, mod = "Fertilisation_mode", group = "Study_ID", data = dat3, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45)) +
  ggtitle('Sperm velocity') +
    geom_signif(comparisons = list(c("internal", "external")),
              annotations = "*")

Fig.S6 <- (figS6a+figS6b)/(figS6c + figS6d + figS6e) + plot_annotation(tag_levels = 'A')
Fig.S6
...
calculate_ci_for_multiple_observations <- function(group_data) {
  if (nrow(group_data) > 1) {
    ci <- t.test(group_data$yi)$conf.int
    return(c(lower = ci[1], upper = ci[2]))
  } else {
    return(NA)  # Return NA for groups with only one observation
  }
}

dat4_ci_multiple_observations <- dat4 %>%
  group_by(Species) %>%
  summarize(
    mean_zr = mean(yi),
    ci_lower = ifelse(n() > 1, calculate_ci_for_multiple_observations(.)['lower'], NA),
    ci_upper = ifelse(n() > 1, calculate_ci_for_multiple_observations(.)['upper'], NA)
  ) %>%
  ungroup()

Figs7 <- facet_plot(plottree, panel="dot", data=dat4_averageeffect, geom=geom_point, aes(x=mean_zr), size=2.5) +
...
# Figs7 <- facet_plot(plottree, panel="dot", data=dat4_ci_multiple_observations, geom=geom_point, aes(x=mean_zr), size=2.5) +
#   theme(axis.title.x=element_text(size=10),
#         axis.text.x=element_text(size=10)) +
#   geom_vline(xintercept=0) +
#   geom_errorbar(aes(x=mean_zr, ymin=ci_lower, ymax=ci_upper), width=0.2, position=position_dodge(width=0.25))
...
Figs8 <- facet_plot(plottree, panel="dot", data=dat5_averageeffect, geom=geom_point, aes(x=mean_zr), size=2.5) +
  theme(axis.title.x=element_text(size=10),
        axis.text.x=element_text(size=10)) +
  geom_vline(xintercept=0) 
...
## Fig.S9
```{r}
funnel(mod_fm, xlab = "Zr", ylab = "Standard error", yaxis = "seinv")
```
