...
### Sub-analysis on only fish
#### Global model
```{r,}
dat6 <- subset(dat, Class == "Actinopterygii")

VCV7 <- impute_covariance_matrix(vi = dat6$vi, cluster = dat6$Study_ID, r = 0.5)

m7 <- rma.mv(yi = yi, V = VCV7, mod = ~Fertilisation_mode-1,  
             random = list(~1|Study_ID,
                          ~1|Effectsize_ID,
                          ~1|Species),
             test = "t",
...
#TODO need to inset phylogeny back into model. DOUBLE CHECK K for fert mode
