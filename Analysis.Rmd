...
#   tip = mytree_spermlength$tip.label,
#   label = mytree_spermlength$tip.label)

# tip_labels$tip <- gsub('_',' ',tip_labels$tip)
# tip_labels$label <- gsub('_',' ',tip_labels$label)

plottree <- ggtree(mytree_spermlength) + 
  geom_tiplab(size = 3, hjust = -0.1, align = TRUE)

#can't get ggtree to plot tiplabels
# plottree <- plottree +
#   geom_tiplab(data = tip_labels, aes(label = label, x = tip), size = 3, hjust = -0.1, geom = "text", offset = 2)
...
d1 <- data.frame(id=mytree_spermlength$tip.label, zr=dat4_averageeffect$mean_zr)

#TODO can't figure out how to fit tree into facet
Figs5 <- facet_plot(plottree, panel="dot", data=d1, geom=geom_point, aes(x=zr), size=2.5) +
