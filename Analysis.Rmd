...
fig_fisha <- orchard_plot(res_fisha, mod = "PC_type", group = "Study_ID", data = dat_Classa, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh") +
  theme(axis.text.y = element_text(angle = 45)) +
...
fig <- orchard_plot(res_fish_velocity, mod = "Fertilisation_mode", group = "Study_ID", data = dat3, xlab = "Correlation (r)", alpha=0.4, transfm = "tanh")+ ylim(-1,1.25) +
  theme(axis.text.y = element_text(angle = 45)) +
  ggtitle('Sperm velocity') +
    geom_signif(comparisons = list(c("internal", "external")),
              annotations = "*")
